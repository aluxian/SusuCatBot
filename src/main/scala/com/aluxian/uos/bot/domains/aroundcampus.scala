...
object AroundCampusResponses extends ResponseGenerator {
  def default(): String = {
    s"Meow, please be more specific or ask me something else, I'm a cat, not a robot! :o"
  }
  def eat(): String = {
    s"If you were a cat :V, you could eat wherever you want! But, as you are probably human :(, you can eat on campus in Piazza, The Stags, The Bridge, or get a nice sandwich from The Shop!"
  }
  def bathroom(): String = {
    s"If you are in Mountbatten building, and I know you are since I am a cat :D, its on 4th floor, just outside of the common room on the right!"
  }
  def beer(): String = {
    s"Check out The Stag's, its the cheapest pub EVER! :o I hang out there all the time with my tomcats! :$"
  }
...
object AroundCampusStory extends Story {
  val phrases = List(
    ...
    "Where is the nearest bathroom?" \\ EntityDef.Intent.GetBahtroom,
  ...
  def run(past: BotPast, bot: BotInterface): Future[List[BotAction]] = {
    ...
    if (intentOpt.isDefined) {
      ...
      if (intent equals Intent("get_eat")) {
        return Future(List(Respond(TextBotResponse(FunResponses.eat()))))
      } else if (intent equals Intent("get_bathroom")) {
        return Future(List(Respond(TextBotResponse(FunResponses.bathroom()))))
      } else if (intent equals Intent("get_beer")) {
        return Future(List(Respond(TextBotResponse(FunResponses.beer()))))
      } 
    }
